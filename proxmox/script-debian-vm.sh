...
function advanced_settings() {
  ...
  if RAM_GB=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Allocate RAM in GiB (Available: $available_ram_gb GB out of $total_ram_gb GB)" 8 58 2 --title "RAM" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z "$RAM_GB" ]; then
      RAM_GB="2"
    fi

    # Converteer RAM van GB naar MB en bewaar in de bestaande variabele RAM_SIZE
    RAM_SIZE=$((RAM_GB * 1024))

    echo -e "${DGN}Allocated RAM: ${BGN}${RAM_GB} GiB (${RAM_SIZE} MiB)${CL}"
  else
    exit-script
  fi
...
function advanced_settings_cloudinit() {
  # Get the total RAM in kB available on the host
  total_ram_kb=$(grep MemTotal /proc/meminfo | awk '{print $2}')
  total_ram_mb=$((total_ram_kb / 1024))
  total_ram_gb=$((total_ram_mb / 1024))
  ...
  if VM_NAME=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Hostname (leave blank for default: "debian")" 8 58 --title "HOSTNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      HN="debian"
      echo -e "${DGN}Using Hostname: ${BGN}$HN${CL}"
    else
  ...
  if USERNAME=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Username (Default: "debian")" 8 58 --title "USERNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    # Voeg een invoermenu toe voor de variabele 'PASSWORD'
    if PASSWORD=$(whiptail --backtitle "Proxmox VE Helper Scripts" --passwordbox "Set Password (Default: "B4dP455")" 8 58 B4dP455 --title "PASSWORD" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
      if [ -z "$USERNAME" ]; then
        USERNAME="debian"
      fi
      echo -e "${DGN}Using Username : ${BGN}$USERNAME${CL}"
      if [ -z "$PASSWORD" ]; then
        PASSWORD="B4dP455"
      fi
      echo -e "${DGN}Using Password : ${BGN}$PASSWORD${CL}"
    else
      exit-script
    fi
  else
  ...
  if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create a Debian 12 VM?" --no-button Do-Over 10 58); then
    ...
    header_info
    echo -e "${RD}Using Advanced Settings with CloudInit${CL}"
    advanced_settings_cloudinit
  fi
...
function start_script() {
  if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "SETTINGS" --yesno "Use Default Settings?" --no-button Advanced 10 58); then
    ...
    header_info
    if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "SETTINGS" --yesno "Advanced Settings with or Without CloudInit?" --yes-button "With CloudInit" --no-button "Without CloudInit" 10 58); then
      echo -e "${RD}Using Advanced Settings with Cloudinit${CL}"
      advanced_settings_cloudinit
    else
      echo -e "${RD}Using Advanced Settings without Cloudinit${CL}"
      advanced_settings
    fi
  fi
}
...
if [ -n "$USERNAME" ] && [ -n "$PASSWORD" ]; then
  msg_info "Setting additional settings for CloudInit"
  qm set $VMID \
    ...
    -vga serial0 \
    -ciuser ${USERNAME} \
    -cipassword ${PASSWORD} >/dev/null
  qm set $VMID \
    -ipconfig0 ip=dhcp \
    -searchdomain tl \
    -nameserver 10.0.1.111 >/dev/null
  msg_ok "Set additional settings for CloudInit"
fi
