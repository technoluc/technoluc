...
function advanced_settings() {
  ...
  if RAM_GB=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Allocate RAM in GiB" 8 58 2 --title "RAM" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z "$RAM_GB" ]; then
      RAM_GB="2"
    fi
...
function advanced_settings_cloudinit() {
  ...
  if USERNAME=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Username" 8 58 debian --title "USERNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    # Voeg een invoermenu toe voor de variabele 'PASSWORD'
    if PASSWORD=$(whiptail --backtitle "Proxmox VE Helper Scripts" --passwordbox "Set Password" 8 58 B4dP455 --title "PASSWORD" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
      if [ -z "$USERNAME" ]; then
        USERNAME="debian"
      fi
      if [ -z "$PASSWORD" ]; then
...
if [ -n "$USERNAME" ] && [ -n "$PASSWORD" ]; then
  msg_info "Additional steps for CloudInit"
  qm set $VMID \
